...
- 使用前确认本机有 `tesseract`，且中文语言包 `chi_sim` 可用；没有时仍可按 `ingredient_acquisition.md` 手工搜索和人工读图。
- OCR 命中的“配料/原料/成分分析”必须人工复核 SKU、图片清晰度和上下文，不能把 OCR 错字、卖点图或相似 SKU 当作准确背标。
- 改动配料查找流程时，同步更新 `ingredient_acquisition.md`、`eval_cases.md` 和 `eval_cases.yaml` 中的商品用例。
