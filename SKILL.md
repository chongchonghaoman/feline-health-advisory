...
2. **权威标准做法**：说明依据的机构或指南，不编造精确条文。每条建议必须区分"直接引用指南/机构"和"一般养护常识"——后者须标注"此为一般养护建议，非直接引用指南条文"。当注册来源未覆盖具体主题时，明确说明"当前注册来源未覆盖此细分主题"，不可将通用知识伪装成权威结论。
